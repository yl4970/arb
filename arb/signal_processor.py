...
@apply_to_data
def load_all(df, threshold=50, latency_lmt=10, transaction_fee=50):
    df = filter_trading_hours(df)
    df = filter_na_exchange(df)
    df = filter_actions(df)
    df = reshape_bid_ask_data(df)
    return process_signals(df, threshold=threshold, latency_lmt=latency_lmt, transaction_fee=transaction_fee)
...
@apply_to_data
def process_signals(df, threshold=50, latency_lmt=10, transaction_fee=50):
    """
    ...
    Parameters:
        ...
        latency_lmt (int): The minimum duration (in rows) for a valid signal.
    ...
    def get_signal(signal_start, signal_end, signal_side, sum_q, current_ex, latest_price):
        ...
        if signal_end - signal_start >= latency_lmt:
            ...
            if not np.isnan(stale_price) and current_price + stale_price > transaction_fee / 100:
                key = (
                    df.iloc[signal_end].name-df.iloc[signal_start].name, # latency
                    current_ex,
                    stale_exchange
                )
                return {
